import{W as Ct,r as y,aP as zt,o as Et,u as At,a as It,q as Dt,g as U,aG as Mt,j as e,Q as Qe,c as be,d as Ye,F as Ot,S as oe,m as P,ab as Tt,h as $t,e as A,at as Ee,f as Ae,T as Ke,a0 as _t,A as ce,i as K,w as Xe,X as Ie,l as De,n as Z,s as de,p as ye,L as ve,t as we,v as Rt,k as je,O as he,aQ as Me,I as Lt,x as Ft,y as Ht,z as Ze,C as Je,aa as Pt}from"./index-COc8314T.js";import{P as Gt}from"./pdfService-Bp0i6AeL.js";import{Q as qt}from"./QRScanner-CK92mbH0.js";import{D as et}from"./download-Jz4YpU7U.js";import{P as Ut}from"./plus-Dq5F4M9d.js";import{F as Wt}from"./funnel-D1v7viCl.js";import{S as Bt}from"./square-pen-DMiSdvLO.js";import{R as Vt}from"./rotate-ccw-SBZQZCB0.js";import{C as Qt}from"./chevron-up-gxB51Db0.js";import"./jspdf.plugin.autotable-eB7zR5oO.js";/**
...
فشل: ${n} مادة`)},xt=async(t,l)=>{try{await Ft(Z(U(m,"chemicals"),t)),await ye(m,ve.DELETE,we.CHEMICALS,`حذف المادة: ${l}`,t),(o==null?void 0:o.id)===t&&S(i.find(n=>n.id!==t)||null)}catch(n){je(n,he.DELETE,`chemicals/${t}`)}},ft=()=>{const t=window.open("","_blank");if(!t){alert("يرجى السماح بالنوافذ المنبثقة لطباعة القائمة");return}const l=Q.filter(b=>b.ghs&&b.ghs.length>0||b.hazardClass==="danger").length,p=new Date().toLocaleDateString("ar-DZ",{day:"2-digit",month:"2-digit",year:"numeric"}),x="2025/2026",u=Q.map((b,R)=>{const re=b.ghs&&b.ghs.length>0||b.hazardClass==="danger",ne=(b.ghs||[]).map(G=>`<div class="ghs-pic"><img src="${ee[G]}" alt="${G}" /></div>`).join("");return`
    ...
    `),t.document.close()},gt=async()=>{const t=["#","الاسم العلمي","الاسم العربي","الصيغة","الكمية","الرف","تاريخ الصلاحية"],l=$.map((n,p)=>[p+1,n.nameEn||"",n.nameAr||"",n.formula||"",`${n.quantity} ${n.unit}`,n.shelf||"",ke(n.expiryDate)]);await Gt.generateTablePDF("تقرير جرد المواد الكيميائية",t,l,`chemicals_inventory_${new Date().toISOString().split("T")[0]}.pdf`)},bt=()=>{const t=K.json_to_sheet($.map(n=>({"الاسم (EN)":n.nameEn,"الاسم (AR)":n.nameAr,الصيغة:n.formula,"رقم CAS":n.casNumber,الكمية:n.quantity,الوحدة:n.unit,الحالة:n.state,الخطورة:n.hazardClass,الرف:n.shelf,"تاريخ الصلاحية":n.expiryDate,ملاحظات:n.notes}))),l=K.book_new();K.book_append_sheet(l,t,"Inventory"),Xe(l,`chemical_inventory_${new Date().toISOString().split("T")[0]}.xlsx`)},yt=async t=>{var p;const l=(p=t.target.files)==null?void 0:p[0];if(!l)return;me(!0);const n=new FileReader;n.onload=async x=>{var u;try{const b=(u=x.target)==null?void 0:u.result,R=Ht(b,{type:"binary",cellDates:!0}),re=R.SheetNames[0],ne=R.Sheets[re],G=K.sheet_to_json(ne),xe=N=>{if(!N)return"";if(N instanceof Date)return N.toISOString().split("T")[0];const X=new Date(N);return isNaN(X.getTime())?String(N).trim():X.toISOString().split("T")[0]},L=(N,X)=>{const Ce=Object.keys(N);for(const ie of X){const le=Ce.find(q=>q.toLowerCase().trim()===ie.toLowerCase().trim());if(le)return N[le]}},Ve=Ze(Je);G.forEach(N=>{const X=L(N,["PRODUIT CHIMIQUE","Name","nameEn","Product","Chemical"])||"Unnamed Chemical",Ce=L(N,["الاسم العربي","الاسم","Arabic Name","nameAr","Arabic"])||"",ie=L(N,["الكمية","Quantity","quantity","Qty","Amount"]),le=typeof ie=="number"?ie:parseFloat(String(ie||"0").replace(/[^0-9.]/g,""));let q=String(L(N,["الحالة","State","state","Status"])||"solid").trim(),fe="solid";q==="صلب"||q.toLowerCase()==="solid"?fe="solid":q==="سائل"||q.toLowerCase()==="liquid"?fe="liquid":(q==="غاز"||q.toLowerCase()==="gas")&&(fe="gas");let ge=String(L(N,["الخطورة","Hazard","hazardClass","Danger"])||"safe").trim(),ze="safe";ge==="خطر"||ge.toLowerCase()==="danger"?ze="danger":(ge==="آمن"||ge.toLowerCase()==="safe")&&(ze="safe");const kt=Z(U(m,"chemicals"));Ve.set(kt,{nameEn:String(X).trim(),nameAr:String(Ce).trim(),formula:L(N,["الصيغة","Formula","formula"])||"",unit:L(N,["الوحدة","Unit","unit"])||"g",quantity:isNaN(le)?0:le,state:fe,hazardClass:ze,ghs:Array.isArray(N.GHS)?N.GHS:N.GHS?String(N.GHS).split(",").map(St=>St.trim()):[],shelf:L(N,["الرف","Shelf","shelf"])||"",expiryDate:xe(L(N,["الصلاحية","Expiry","تاريخ الانتهاء","expiryDate"])),notes:L(N,["ملاحظات","Notes","notes"])||"",createdAt:de()})}),await Ve.commit(),alert(`تم استيراد ${G.length} مادة بنجاح!`)}catch(b){console.error("Error importing XLS:",b),alert("حدث خطأ أثناء استيراد الملف. يرجى التأكد من صيغة الملف.")}finally{me(!1),H.current&&(H.current.value="")}},n.readAsBinaryString(l)},Se=t=>{const l=window.open("","_blank");if(!l){alert("يرجى السماح بالنوافذ المنبثقة لطباعة البطاقات");return}const n=new Date,p="2025/2026",x=t.map((u,b)=>{var G,xe;const R=u.state==="solid"?"صلب":u.state==="liquid"?"سائل":"غاز",re=u.hazardClass==="danger"?(G=u.ghs)!=null&&G[0]?pe[u.ghs[0]]:"خطر":"آمن",ne=(xe=u.ghs)!=null&&xe[0]?"☠️":"—";return`
        <div class="pcard">
